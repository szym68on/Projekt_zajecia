...
#### 4. Generate DynamicScore Data 🔄
```powershell
cd src\python
python dynamic_score_calculator.py
cd ..\..
```

#### 5. Build and Run Visualization 🚀
```powershell
cd visualization
mvn clean install
mvn exec:java
...
```bash
cd src/python
python3 dynamic_score_calculator.py
cd ../..
```
...
1. **Select Club** 🏟️: Choose from dropdown (Athletic Bilbao, Atlético Madrid, etc.)
